...
We structure the derivation of the forward upate equation for the Markov model's posterior distribution $P(x_{t+1}, \alpha, \beta \mid y_{1:{t+1}})$ as given by \citet*{russell2002artificial}:
\begin{align}
...
\end{align}
where we have applied Bayes' theorem, discarded a normalising constant, and used the conditional dependence structure of our model. We computed an expression for the first factor, the likelihood $P(y_{t+1} \mid x_{t+1}, \alpha, \beta)$, earlier. The second factor $P(x_{t+1}, \alpha, \beta \mid y_{1:t})$ needs to be rewritten as some function of $P(x_{t}, \alpha, \beta \mid y_{1:t})$. We condition on $x_t=s, \alpha=a, \beta=b$:
\begin{align}

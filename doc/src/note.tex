...
The goal is to derive a computationally tractable way to update the posterior of the latent variables $(x_t, \lambda)$ given the information from one new observation of an event count $y_{t+1}$. An exact update would be some function $F$ expressing the following relation:
\begin{equation}
P(x_{t+1}, \alpha, \beta \mid y_{1:t+1}) = F\left( P(x_t, \alpha, \beta \mid y_{1:t}), y_{t+1}\right)
\end{equation}

We structure the derivation of the exact forward upate equation for the model's posterior distribution $P(x_{t+1}, \alpha, \beta \mid y_{1:{t+1}})$ following \citet*{russell2002artificial}:
\begin{align}
P(x_{t+1}, \alpha, \beta \mid y_{1:{t+1}})
& = P(x_{t+1}, \alpha, \beta \mid y_{1:t}, y_{t+1}) \nonumber \\
& = \frac{P(y_{t+1} \mid x_{t+1}, \alpha, \beta, y_{1:t}) P(x_{t+1}, \alpha, \beta \mid y_{1:t})}{P(y_{t+1} \mid y_{1:t})} \nonumber \\
& \propto P(y_{t+1} \mid x_{t+1}, \alpha, \beta, y_{1:t}) P(x_{t+1}, \alpha, \beta \mid y_{1:t}) \nonumber \\
& = P(y_{t+1} \mid x_{t+1}, \alpha, \beta) P(x_{t+1}, \alpha, \beta \mid y_{1:t}) , \label{eqn:rawforward}
\end{align}
where we have applied Bayes' theorem, omitted a normalising constant, and used the conditional dependence structure of our model. We prepared an expression for the likelihood $P(y_{t+1} \mid x_{t+1}, \alpha, \beta)$ earlier. The second factor $P(x_{t+1}, \alpha, \beta \mid y_{1:t})$ needs to be rewritten as some function of $P(x_{t}, \alpha, \beta \mid y_{1:t})$. We condition on $x_t=s, \alpha=a, \beta=b$:
\begin{align}
& P(x_{t+1}=s^{\prime}, \alpha=\alpha^{\prime}, \beta=\beta^{\prime} \mid y_{1:t}) \nonumber \\
= & \sum_{s \in S} \int_{a, b} P(x_{t+1}=s^{\prime}, \alpha=\alpha^{\prime}, \beta=\beta^{\prime} \mid x_t=s, y_{1:t}, \alpha=a, \beta=b ) P(x_t=s, \alpha=a, \beta=b \mid y_{1:t}) \,d(a, b) \nonumber \\
= & \sum_{s \in S} \int_{a, b} P(x_{t+1}=s^{\prime} \mid x_t=s) P(\alpha=\alpha^{\prime} \mid \alpha = a) P(\beta=\beta^{\prime} \mid \beta = b) P(x_t=s, \alpha=a, \beta=b \mid y_{1:t}) \,d(a, b) \nonumber \\
= & \sum_{s \in S} P(x_{t+1}=s^{\prime} \mid x_t=s) P(x_t=s, \alpha=\alpha^{\prime}, \beta=\beta^{\prime} \mid y_{1:t}) \nonumber \\
= & \sum_{s \in S} A_{s^{\prime}, s} P(x_t=s, \alpha=\alpha^{\prime}, \beta=\beta^{\prime} \mid y_{1:t} ) \label{eqn:advance}
\end{align}
Combining the proportional relation~\ref{eqn:rawforward} with equation~\ref{eqn:advance} and the expression for the likelihood~\ref{eqn:hybridobs} gives
\begin{align}
& P(x_{t+1}=s^{\prime}, \alpha=\alpha^{\prime}, \beta=\beta^{\prime} \mid y_{1:{t+1}}) \nonumber \\
\propto &
\left[ B_{k,s^{\prime}} * \negbind(k ; \alpha^{\prime}, \beta^{\prime}) \right] \Bigr \rvert_{k=y_{t+1}}
\sum_{s \in S} A_{s^{\prime}, s} P(x_t=s, \alpha=\alpha^{\prime}, \beta=\beta^{\prime} \mid y_{1:t} ) \, .
\end{align}
